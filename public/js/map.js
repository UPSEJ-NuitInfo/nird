...
const levels = [
  {
    ...
    name: 'Dino Google',
    description:
      'Esquivez les obstacles et progressez dans le jeu du dinosaure',
    icon: '🦖',
  },
  {
    id: 2,
  ...
  if (!gameState.completedLevels.includes(levelId)) {
    ...
    setTimeout(() => {
      if (currentUser) {
        ...
        alert(
          `✨ Score enregistré pour ${currentUser.username} (${pendingLevelScore})`,
        );
      } else {
...
async function showLeaderboard(levelId = null) {
  ...
  leaderboardData.sort(
    (a, b) => (b.score || b.xp || 0) - (a.score || a.xp || 0),
  );
...
function displayLeaderboard(levelId) {
  ...
  if (!leaderboardData || leaderboardData.length === 0) {
    ...
    leaderboardData.slice(0, 50).forEach((user, index) => {
      ...
      const displayScore =
        user.score != null ? `${user.score}` : `${user.xp} XP`;
...
function renderProfileArea() {
  ...
  if (currentUser) {
    ...
    content.innerHTML = `
            ...
            <p><strong>XP:</strong> <strong style="color:var(--primary);">${
              currentUser.xp || 0
            }</strong></p>
        `;
